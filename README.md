...
OCaml uses the term *domain* which roughly corresponds to a thread. We use terms *multicore*, *multidomain*, and *parallel* to mean the same thing. Other programming languages use the term *multithreaded* to refer to the same concept.

### Multidomain vs multiprocess vs concurrent

There exist multiple paradigms to speed up or structure computations. We'll briefly describe how multidomain computing differs from some of them, multiprocess and concurrent computing.
...
**Data race** is a condition when two or more domains access the same memory location, and at least one of them performs a write to that memory location. Given a variable, a read + write and a write + write will produce a data race, while a read + read without any other writes will not.
...
In the program above, the value of `x` could be `1`, `2`, or anything else and some other unexpected things might happen, depending on a compiler and language guarantees.

### Multicore OxCaml guarantees data race freedom

OxCaml brings an extension to multicore OCaml to ensure data race freedom. That is, there exists a **compile time** guarantee that no data race will occur in the program. This is achieved by introducing *portability* and *contention* modes.

*Modes* are a set of annotations that constrain how a value may be used. Modes are checked at compile time. There exist at least nine separate mode axes in OxCaml, but we will focus on two of them: *portability* (portable and nonportable modes) and *contention* (contended and uncontended modes).

### Fork-join model

OxCaml uses a fork-join model for multidomain execution. Under this model's API, we can submit functions to be executed in parallel, and the library code will work out *scheduling* of these computations. That is, it will decide which domains will run which computations.
...
We also pass `par: Parallel.t`, which is a handler that allows the scheduler to control the parallel computations.
...
```
#### Structure
Note that we use `Parallel.fork_join2` to spawn two computations, but there exist `fork_join3`, `4` and `5`, or plain `fork_join` which accepts an `HList` of any number of computations to run in parallel.
...
### Thread-preservation property
OxCaml's **fork-join** library **preserves the domain of execution for the first argument** to the `Parallel.fork_join*` function.
...
On the diagram below, the lines marked `domain1`, `domain2`, and `domain3` trace computations that will be executed on the same domain, for each of the lines.
...
Indeed, this program contains a data race on the `count` variable. We'll learn to decipher such compiler messages in a moment, but for now let's think of all the ways to fix this program.

We can add a mutex, make `count` an atomic variable, or use local counters. In this tutorial, we'll focus on the latter approach and only cover programs **without shared mutable state**. Atomics and synchronization primitives (such as a mutex) will be covered in a later tutorial.

With local counters, we can reuse the sequential version and rewrite the parallel version as follows. As a bonus, this program now even looks more functional-style.
...
Contention mode only applies to values that contain a **mutable state**. Indeed, an immutable value can always be safely accessed from multiple domains, so no annotations are required to ensure data-race freedom. Thus, we say **immutable values cross contention** to mean that immutable values can be treated both as contended and uncontended values.
...
Immutable values are often used in the contended mode, since this is the most restrictive mode which gives the most guarantees. The immutable values do not need to claim any of the privileges provided by the uncontended mode: they cannot be written to because they are immutable, and they can be read from by any domain because they cross contention.

Recall that `ref` is a record with a single mutable field.

```ocaml
val (x @ contended) = 42
val (x @ uncontended) = 42

val (lst @ contended) = [1; 2; 3]
val (lst @ uncontended) = [1; 2; 3]

val r_read_only = ref 0 @ contended
val r = ref 0 @ uncontended

val lst = [ref 0; ref 0; ref 0] @ contended
val lst_read_only = [ref 0; ref 0; ref 0] @ uncontended
```

```ocaml
let f1 (lst @ contended) = List.size lst 
let f2 (lst @ uncontended) = List.size lst
```
...
Let's take a look at some functions and values possibly containing functions, like records, tuples, arrays, variants.
```ocaml
...
let lst = [f; f; f]  (* a list is a record *)
...
- `@ uncontended` is a privilege to read or write to a value.
...
For example, a program where one domain modifies a global value `r`, and no other domains access `r` is a correct, data-race free program.
```ocaml
(* Does not compile *)
...
This helps us rewrite our example above to the following. Here `r := 42; !r` in nonportable, but `fork_join*` is ok with this.
```ocaml
(* Compiles *)
let run (par : Parallel.t) =
  let r = ref 0 in
  let #(r1, r2) =
    Parallel.fork_join2 par
      ...
      (fun _par -> 1729)
  in
  r1, r2  (* result: (42, 1729) *)
;;
```

The rest of the arguments to `fork_join*` required to be portable, since they can be run on any domain. However, if they call `fork_join*` themselves, their first argument to that inner `fork_join*` could again be nonportable, modifying some local mutable state created within that branch. Here `r := 42; !r` and `inner_r := 17; !inner_r` are both nonportable.
...
let run (par : Parallel.t) =
  ...
  let #(r1, r2) =
    Parallel.fork_join2 par
      (fun _par -> r := 42; !r)
      (fun _par -> let inner_r = ref 0 in 
                     let #(inner_r1, inner_r2) = 
                     Parallel.fork_join2 par 
                       (fun _par -> inner_r := 17; !inner_r) 
                       (fun _par -> 29)
                     in
                     inner_r1, inner_r2)
  in
  r1, r2  (* result: (42, (17, 29))) *)
;;
```

If the first branch of the inner fork-join calls `fork_join*` again, it can also pass a nonportable function, modifying `inner_r`, to its first argument, thus stretching this value through two forks.

The bottom line is that **all the functions used in `Scheduler.parallel` must be `portable`. The first argument to `fork_join*` can be nonportable,** as long as it remains portable from the `Scheduler.parallel`'s perspective.
...
let run par = calc_weird_stock_tuple_par par stock1 (* result: true false *)
```
...
```ocaml
XX | let run par = calc_weird_stock_tuple_par par stock
                                                  ^^^^^
